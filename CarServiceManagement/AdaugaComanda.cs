...
namespace CarServiceManagement
{
    public partial class AdaugaComanda : Form
    {
        ...
        private void AddOrder_Load(object sender, EventArgs e)
        {
            ...
            string[] allRequestTypes = { "Livrare", "Ridicare" };
        ...
        private async void add_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("Va rugam completati toate spatiile");
                }

                int totalCostCasted;
                if(!int.TryParse(totalCost, out totalCostCasted))
                {
                    throw new Exception("Va rugam introduceti doar numere in acest spatiu");
                }

                if(requestType == "Dropoff" && ownerAddress.Length == 0)
                {
                    throw new Exception("Adresa este necesara pentru comenzile cu livrare");
                }
